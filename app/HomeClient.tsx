...
import { items, categories, locationTypes, valueTiers, type Item } from "@/lib/items-data";
import { ItemImage } from "@/components/ItemImage";

const FeaturedYoutubeVideos = dynamic(() => import("@/components/FeaturedYoutubeVideos"), { ssr: false });

type ActionId = 'all' | 'keep' | 'sell' | 'recycle';
...
export default function HomeClient() {
  ...
  const [selectedAction, setSelectedAction] = useState<ActionId>('all');
  ...
  const filteredItems = useMemo(() => {
    ...
    if (selectedLocation !== 'all') {
      const locationFilter = selectedLocation as NonNullable<Item['locationTypes']>[number];
      filtered = filtered.filter(item =>
        item.locationTypes && item.locationTypes.includes(locationFilter)
      );
    }
